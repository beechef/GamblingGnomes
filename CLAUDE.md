...
- **UI prefabs split into `Prefabs/UI/Components/` and `Prefabs/UI/Screens/`.** A component is a piece worth reusing — `UI_MenuItem`, `UI_ProgressBar`, `UI_SelectionPointer`, `UI_TitleLabel`, `UI_Background`; a screen composes them as nested prefab instances, so restyling every menu entry in the game is one edit to one prefab. A screen root is full-screen `(0,0)`–`(1,1)` with zero offsets and carries its own `CanvasGroup`.
