...
{
	// What a unit of stake can be. A unit replicates as its PokerBetItemType — the same trade
	...
	// in the pot.
	[CreateAssetMenu(fileName = "PokerBetItemDatabase", menuName = "Game/Poker/Bet Item Database")]
	public class PokerBetItemDatabase : ScriptableObject
	{
		public const PokerBetItemType PlainChip = PokerBetItemType.PlainChip;
		...
		public class Entry
		{
			...
			[SerializeField] private PokerBetItemEffect _effect;

			[Tooltip("Off, this kind can never be staked — it is only ever fed to somebody on purpose. The Colorful cap is the case.")]
			...
			public PokerBetItemEffect Effect => _effect;
			public bool Bettable => _bettable;
		}

		[InfoBox("Each row names the kind it describes, so the order here is only a reading order. What must never change is a kind's value in PokerBetItemType — that is what replicates.")]
		...
		public PokerBetItemType DrawBetItemType()
		{
			var totalWeight = 0;
			foreach (var entry in _entries)
			{
				// A kind nobody may bet is not one the table deals at random either.
				if (entry != null && entry.Bettable) totalWeight += entry.Weight;
			}
		...
		public bool TryGetEntry(PokerBetItemType itemType, out Entry entry)
		{
			entry = null;
