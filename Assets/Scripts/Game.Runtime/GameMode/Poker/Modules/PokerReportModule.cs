...
{
	// The accusation game, plugged into the table as one module: who may point at whom, what it costs in
	// blood, and how the verdict is announced. Guilt lives server-side in plain collections — replicating
	// it would hand the answer to the very players supposed to be guessing.
	//
	// It owns none of the abilities any more: dealing them, holding them and spending them belong to
	// PokerAbilityModule, and this asks that module the one question it needs — whether the accused had
	// in fact played a cheat. Hold'em is the only preset that runs both.
	public class PokerReportModule : PokerModule
	{
		[Header("Abilities")]
		[Tooltip("The module that deals and spends abilities. Asked one question only: whether the accused had played a cheat. Empty makes every accusation a wrong one, which is a table where pointing at people costs blood and proves nothing.")]
		[SerializeField] private PokerAbilityModule _abilities;
		...
		protected override void OnCollectConfigEntries(List<MatchConfigEntry> entries)
		{
			...
			entries.Add(new MatchConfigInt(ModuleId, "Report", "ReportsPerHand", "Reports Per Hand", 0, 5, 1,
		...
		{
			if (stage is not PokerDealStage) return;

			ClearRoundServer();
			GrantChargesServer();
		}

		// Each hand gives everybody dealt in their accusations back. This used to ride the ability deal,
		// and moving that out left nobody handing them out — a table where the first hand could be
		// reported on and no other could, with nothing logged to say why.
		private void GrantChargesServer()
		{
			...
			foreach (var player in GameMode.SeatedPlayers)
			{
				...
				player.Data.ReportsLeft.Value = _reportsPerRound;
			}
		}
		...
		public void ResolveReportServer()
		{
			...
			var wasCheater = _abilities && _abilities.HasPlayedCheat(_pendingTarget);
