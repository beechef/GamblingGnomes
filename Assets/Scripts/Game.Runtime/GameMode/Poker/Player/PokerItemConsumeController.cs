...
namespace Game.Runtime.GameMode.Poker.Player
{
	...
	public class PokerItemConsumeController : NetworkBehaviour
	{
		...
		public event Action<PokerItemConsumeController> OnPlateFinished;
		...
		public bool HasPlate
		{
			get
			{
				...
				return CanEat && mode && mode.Data && PokerTableUtility.CountPotItems(mode.Data, _player.ClientId) > 0;
			}
		}
		...
		public void ServerStopEating()
		{
			...
			if (_pendingItems.Count > 0) ApplyPendingEffects();
		...
		private bool TakeBite()
		{
			...
			if (!data || PokerTableUtility.CountPotItems(data, _player.ClientId) == 0) return false;

			_pendingItems.Clear();

			if (_rule.EatWholePlate) TakeWholePlate(data);
			else TakeItems(data, _rule.ItemsPerBite);
		...
		private void TakeItems(PokerGameData data, int count)
		{
			for (var i = 0; i < count; i++)
			{
				if (!PokerTableUtility.ServerTakePotItem(data, _player.ClientId, out var itemType)) break;
		...
		{
			while (PokerTableUtility.ServerTakePotItem(data, _player.ClientId, _rule.IsEatenWithOthers, out var itemType))
			{
				_pendingItems.Add(itemType);
			}
			...
			if (PokerTableUtility.ServerTakePotItem(data, _player.ClientId, _rule.IsEatenOnItsOwn, out var alone))
			{
